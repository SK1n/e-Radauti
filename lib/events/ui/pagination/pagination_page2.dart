...
class HomeState3 extends State<HomePage3> {
  GlobalKey<PaginatorState> paginatorGlobalKey = GlobalKey();
  ...
  Future<CountriesData3> sendCountriesDataRequest(int nr) async {
    try {
      String url = Uri.encodeFull(
        //'http://api.worldbank.org/v2/country?page=$nr&format=json'
          'https://data-a2c86.firebaseio.com/items.json'
      ...
      http.Response response = await http.get(url);
      return CountriesData3.fromResponse(response, nr);
    } catch (e) {
      if (e is IOException) {
        return CountriesData3.withError(
            'Please check your internet connection.');
      } else {
        print(e.toString());
        return CountriesData3.withError('Something went wrong.');
      }
    }
  }
  ...
  List<dynamic> listItemsGetter(CountriesData3 countriesData) {
    //List<String> list = [];
    //List list = [];
    Map<String, dynamic> list = {};
    List list2 = [];
  ...
  Widget errorWidgetMaker(CountriesData3 countriesData, retryListener) {
    return Column(
      mainAxisAlignment: MainAxisAlignment.center,
      children: <Widget>[
        Padding(
  ...
  Widget emptyListWidgetMaker(CountriesData3 countriesData) {
    return Center(
      child: Text('No countries in the list'),
    );
  }

  int totalPagesGetter(CountriesData3 countriesData) {
    return countriesData.total;
  }

  bool pageErrorChecker(CountriesData3 countriesData) {
    return countriesData.statusCode != 200;
  }
}

class CountriesData3 {
  //List<dynamic> countries;
  //List<dynamic> events;
  Map<String, dynamic> events;
  int statusCode;
  ...
  CountriesData3.fromResponse(http.Response response, int index) {
    this.statusCode = response.statusCode;
    List jsonData = json.decode(response.body);
    //countries = jsonData[1];
    //countries = jsonData;
  ...
  CountriesData3.withError(String errorMessage) {
    this.errorMessage = errorMessage;
  }
}